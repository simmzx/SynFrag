...
def parse_args():
  ...
  CSV file mode:
    ...
    python farscore.py --input_file molecules.csv --smiles_field "SMILES"
...
def print_smiles_results(smiles_list, scores):
    ...
    print("\n📊 FARScore Results:")
    print(f"{'#':<3} {'SMILES':<30} {'FARScore':<10} {'Status':<10}")
    print("-" * 55)
    ...
    try:
        ...
        if args.input_file:
            ...
            try:
                ...
                if args.output_file:
                    df = pd.DataFrame({'smiles': smiles_list, 'farscore': all_scores[:len(smiles_list)]})
